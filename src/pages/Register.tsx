...
export default function Register() {
  ...
  useEffect(() => {
    const checkUsername = async () => {
      ...
      try {
        ...
        // If there's a permission error, we'll allow it for now to not block sign up
  ...
  const isEmailValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) && !emailAlreadyUsed;
      ...
      <div className="flex-1 overflow-y-auto flex flex-col items-center px-4 py-8">
        ...
        <form onSubmit={handleRegister} className="w-full max-w-sm space-y-3">
          ...
          <div className="relative">
            ...
            {email.length > 0 && (
              <div className="absolute right-3 top-1/2 -translate-y-1/2">
                {isEmailValid && !emailAlreadyUsed ? <CheckCircle2 size={20} className="text-[#00C853]" /> : <XCircle size={20} className="text-[#ED4956]" />}
              </div>
            )}
          </div>
